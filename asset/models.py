...
class AssetLoginUser(models.Model):
    ...
    password = models.CharField(
        ...
        null=True,
        verbose_name='密码')
    private_key = models.FileField(upload_to='upload/privatekey/%Y%m%d{}'.format(
        random.randint(0, 99999)), verbose_name="私钥", null=True, blank=True)
...
class AssetInfo(models.Model):
    ...
    network_ip = models.GenericIPAddressField(
        verbose_name='外网IP', unique=True, null=True, blank=True)
    inner_ip = models.GenericIPAddressField(
        verbose_name='内网IP', null=True, blank=True)
    ...
    expire_time = models.DateTimeField(
        verbose_name='到期时间', null=True, blank=True)
